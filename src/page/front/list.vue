        ...
        <div class="row">
          ...
          <div class="col-12 col-lg-4" v-for="item in list" :key="item.id">
            <div class="single-blog-area mb-50">
              <img v-if="item.cover && item.cover!==''" :src="item.cover" alt="" class="list_img" @click="view(item.id)">
              <img v-if="!item.cover || item.cover===''" src="@/assets/images/core-img/default_cover.jpg" alt="" class="list_img" @click="view(item.id)">
              <!-- Post Title -->
              <div class="post-title-ys">
                {{item.title}}
              </div>
              <div class="list-operation-area">
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      getList (filter) {
        this.$api.post(this.$global.authApiPath, '/learn/list', filter, r => {
          this.list = r.data
          this.cnt = r.cntData
      ...
      del (id) {
        this.$api.get(this.$global.authApiPath, '/learn/del', {id: id}, r => {
          this.getList(this.filter)
        })
      }
    },
